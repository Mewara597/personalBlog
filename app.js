...
mongoose.connect(`mongodb+srv://sid:${process.env.PASS}@cluster0.jdo20sp.mongodb.net/personalBlogDB?retryWrites=true&w=majority`)
...
app.route('/compose').
    ...
    .post(function (req, res) {
        ...
        AppData.findOneAndUpdate({username:req.user.username},{
            $push:{blogs:post}
        },(err)=>{
            if(!err){
                res.redirect('/home')
...
app.get('/delete/:objectID', (req, res) => {
    ...
    AppData.findOneAndUpdate({ username: req.user.username }, {$pull:{blogs:{_id:postId}}},(err)=>{
